...
std::vector<int> GraphAlgorithms::DepthFirstSearch(Graph &graph,
  ...
  while (!stack_.empty()) {
    ...
    stack_.pop();
    if (std::find(visited.begin(), visited.end(), current_vertex) ==
        visited.end()) {
...
std::vector<int> GraphAlgorithms::BreadthFirstSearch(s21::Graph &graph,
                                                     int start_vertex) {
...
GraphAlgorithms::GetShortestPathsBetweenAllVertices(Graph &graph) {
  ...
  std::vector<std::vector<int>> distance(num_verticies,
                                         std::vector<int>(num_verticies, MAX));
...
std::vector<std::vector<int>> GraphAlgorithms::GetLeastSpanningTree(
    Graph &graph) {
  int num_vertices = graph.getNumVertices();
  std::vector<std::vector<int>> mst(num_vertices,
  ...
  while (!priorityQueue.empty()) {
    ...
    for (int neighbor_vertex = 0; neighbor_vertex < num_vertices;
         ++neighbor_vertex) {
...
TsmResult GraphAlgorithms::SolveTravelingSalesmanProblem(Graph &graph) {
  ...
  std::vector<std::vector<int>> antPath(NUM_ANTS,
                                        std::vector<int>(num_vertices + 2));
...
void GraphAlgorithms::AntMovement(
    s21::Graph &graph, const std::vector<std::vector<double>> &pheromoneMatrix,
  ...
  for (int ant = 0; ant < NUM_ANTS; ++ant) {
    ...
    for (int i = 1; i < num_vertices; ++i) {
      ...
      for (size_t j = 0; j < available_vertices.size(); ++j) {
        double pheromone =
            pheromoneMatrix[current_vertex][available_vertices[j]];
        double distance =
            graph.getDistance(current_vertex, available_vertices[j]);
        if (distance != 0) {
          probabilities[j] =
...
void GraphAlgorithms::UpdatePheromone(
    s21::Graph &graph, std::vector<std::vector<double>> &pheromoneMatrix,
...
double GraphAlgorithms::CalculateRouteDistance(
    s21::Graph &graph, const std::vector<int> &antPath) {
...
void GraphAlgorithms::UpdateBestRoute(
    s21::Graph &graph, const std::vector<std::vector<int>> &antPaths,
